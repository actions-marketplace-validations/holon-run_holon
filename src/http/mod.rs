...
#[derive(RustEmbed)]
// web-gui/app/dist is generated by the web build and is git-ignored; it is
// absent from fresh clones and from `cargo package` output. Allow the missing
// folder so the crate compiles with an empty asset set in those trees.
#[allow_missing = true]
